...
* **梯度消失**：在深层网络中，梯度通过链式法则传递时，如果激活函数的导数过小，梯度会逐层减小，最终变为零，导致深层网络无法有效更新。
* **梯度爆炸**：在深层网络中，梯度通过链式法则传递时，如果激活函数的导数较大，或者权重初始化过大，梯度会逐层增大，最终变得非常大，导致训练过程不稳定。


这两种问题都是由**梯度在反向传播中的传播方式**引起的，且往往与**权重初始化**、**激活函数**以及**网络深度**等因素相关。

#### 梯度消失的机理

1. **链式法则的影响**：
   在深层神经网络中，反向传播通过链式法则计算每一层的梯度。假设损失函数是 $L$，输出层的激活是 $y$，并且层与层之间的权重是 $W_i$，则梯度更新的基本公式为：

   $$
   \frac{\partial L}{\partial W_i} = \frac{\partial L}{\partial y} \cdot \frac{\partial y}{\partial x_i} \cdot \frac{\partial x_i}{\partial W_i}
   $$

   其中，$\frac{\partial y}{\partial x_i}$ 是激活函数的导数，$\frac{\partial x_i}{\partial W_i}$ 是输入的梯度。

2. **激活函数的导数过小**：

   * **Sigmoid 函数**：其导数在 $0$ 和 $1$ 处非常小，接近于零。因为当输入值很大或很小时，Sigmoid 的导数值趋近于零，这会导致在反向传播时梯度变得非常小，逐层传递下去时，梯度值逐渐变小，甚至最终接近零，从而使得深层的参数几乎不更新，导致梯度消失。

   * **Tanh 函数**：与 Sigmoid 类似，当输入值过大或过小时，Tanh 的导数也会接近零，导致梯度消失问题。

3. **层数过多**：
   在深层网络中，梯度经过多个层的链式相乘，每一层的梯度都可能会被前一层的梯度缩小。如果激活函数的导数小于 1（如 Sigmoid 和 Tanh），那么梯度在传递过程中就会不断变小，最终在到达最初层时接近零。

4. **影响**：
   梯度消失会导致训练时，**深层的网络权重无法得到有效更新**。结果是，网络只能学习到较浅层的特征，而无法获得深层次的特征，从而影响模型的性能。


#### 梯度爆炸的机理

梯度爆炸问题通常发生在网络的梯度在反向传播过程中变得非常大，导致权重更新过于剧烈，最终导致**网络不稳定**，甚至训练失败。

1. **链式法则的反向传播**：
   类似于梯度消失问题，梯度爆炸的根本原因也在于**链式法则**。在深层网络中，反向传播时梯度被层层传递。如果某一层的权重或者激活函数的导数过大，就可能导致梯度值被不断放大。

2. **权重初始化问题**：

   * 如果网络的**初始权重值过大**，那么在前向传播时，激活值会变得非常大，进而导致反向传播时梯度值的放大。
   * 在反向传播中，如果层的权重过大，或者激活函数的导数较大（比如 ReLU），那么梯度会迅速增大，导致梯度爆炸。

3. **激活函数的影响**：

   * **ReLU**：ReLU 的导数为 1，当输入为正时，梯度保持不变。虽然这有助于避免梯度消失问题，但如果网络的权重初始化过大或者输入数据过大，ReLU 的激活值可能变得非常大，进而导致梯度变得非常大，发生梯度爆炸。
   * **梯度累积**：如果每一层的梯度乘积大于 1，随着层数增加，梯度会不断累积放大，最终导致梯度爆炸。

4. **影响**：
   梯度爆炸会导致**权重更新过快**，导致模型参数超出合理范围，甚至**变得非常大**，从而使得模型的输出不稳定。最终，训练会失败，模型无法收敛。

#### 如何缓解梯度消失和爆炸？

> 是否存在一种方式，使得对于\forall 数据，我都能保证在某种特定的架构中，不会出现梯度消失和爆炸

##### 1. **残差连接（Residual Connection）**

* 核心思想：直接引入「跳跃连接（shortcut）」使梯度可以绕过多层，直接反向传播回去。
* 数学形式：

  $y = F(x) + x$

  与其学习复杂函数 $H(x)$，不如让网络学一个「残差」 $F(x) = H(x) - x$。
* 这样，即使 $F(x) \approx 0$，也能让梯度顺利流动，从而**有效缓解梯度消失问题**。
* 【论文原话】："if identity mappings are optimal, the solvers may simply drive the weights ... toward zero to approach identity mappings"。


##### 2. **批量归一化（Batch Normalization, BN）**

* 作用：保持中间激活值和梯度分布稳定，避免在训练中梯度爆炸或收缩到零。
* BN 会让每层输出具有「零均值、单位方差」的分布，使得训练更加稳定。
* ResNet 在每一层卷积后都加入了 BN。


##### 3. **良好的权重初始化**

* 使用 He 初始化（也叫 Kaiming 初始化）：

  $W \sim \mathcal{N}(0, \sqrt{2 / n})$

  适用于 ReLU 激活函数。
* 这种初始化方法能有效保持正向和反向传播中的信号幅度不会爆炸或消失。

##### 4. **非线性函数选择：ReLU**

* ReLU（Rectified Linear Unit）不会像 sigmoid 那样导致梯度在正向和反向传播中变得非常小（接近 0）。
* 其梯度为常数（1 或 0），便于梯度流动。
